...
void Item::init(glm::ivec2& tileMapPos, ShaderProgram& shaderProgram, Scene* scene) {
	...
	if (itemID == 8) {
		...
		sprite = Sprite::createSprite(glm::ivec2(16, 32), glm::vec2(1, 1), &spritesheet, &shaderProgram);
		sprite->setNumberAnimations(1);
		sprite->setAnimationSpeed(0, 8);
		sprite->addKeyframe(0, glm::vec2(0.f, 0.f));
	}
	else {
		float row = int(itemID % 4)*0.25;
		float col = int(itemID / 4)*0.5;
		...
		sprite->setNumberAnimations(1);
		sprite->setAnimationSpeed(0, 8);
		sprite->addKeyframe(0, glm::vec2(row, col));
	}
